...
async function createApplication(req, res) {
  try {
    ...
    if (hasNewData) {
      ...
      const latestApplication = await dbService.findAndOptionalUpdateProfile(
        keycloakId,
        requestBody?.loanAmount / 26
      );

      if (!latestApplication) {
        return res
          .status(404)
          .json({ message: "Không tìm thấy đơn vay cũ để thực hiện test." });
      }
    ...
    const activeApplication = await dbService.findActiveApplicationByCustomerId(
    ...
    console.log("DEBUG - New Profile ID:", applicationData.profileId);

    const loanApplicationId = await dbService.createLoanApplication(
      applicationData.customer_id,
    ...
    const processInstance = await camundaService.startProcess(
      String(loanApplicationId)
    );
    await dbService.updateApplicationWithProcessId(
      loanApplicationId,
      processInstance.id
    );
