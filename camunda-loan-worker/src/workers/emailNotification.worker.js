...
function registerEmailNotificationWorker(client, sesClient) {
  client.subscribe("emailNotification", async ({ task, taskService }) => {
    ...
    try {
      ...
      switch (type) {
        case "offer":
          ...
          const finalLoanAmount =
            rawLoanAmount < 100000 ? rawLoanAmount * 26 : rawLoanAmount;
          ...
          bodyContent = `
            ...
            <p>
              <a href="http://localhost:3000/accept-offer?applicationId=${
        ...
        case "managerInsights":
          // const insights =
          //   task.variables.get("insights") || "Không có phân tích chi tiết.";

          const insights = "Không có phân tích chi tiết.";

          const taskId = task.id;

          const camundaTaskUrl = `http://main-alb-110239186.us-east-1.elb.amazonaws.com/camunda/app/tasklist/default/#/task/${taskId}`;

          recipientEmail = process.env.MANAGER_EMAIL || "minhsadz@gmail.com";
          subject = `[Cần xem xét] Đơn vay của khách hàng ${
            customerData.full_name || "chưa rõ"
          }`;

          bodyContent = `
                <p>Xin chào Quản lý,</p>
                <p>Hệ thống yêu cầu xem xét thủ công một đơn vay với các thông tin phân tích tự động như sau:</p>
                <blockquote style="border-left: 4px solid #ccc; padding-left: 16px; margin: 16px 0; color: #555;">
                    ${insights}
                </blockquote>
                <p>Vui lòng truy cập vào đường link dưới đây để xem chi tiết hồ sơ và đưa ra quyết định (Phê duyệt/Từ chối):</p>
                <br>
                <a href="${camundaTaskUrl}" 
                   style="display: inline-block; padding: 12px 20px; font-size: 16px; font-weight: bold; color: #ffffff; background-color: #007bff; text-decoration: none; border-radius: 5px;"
                >
                   Xem xét Tác vụ trên Camunda
                </a>
                <br>
                <p>Trân trọng,</p>
        ...
        default:
          console.warn(
            `Loại email không xác định: "${type}". Tác vụ sẽ được hoàn thành mà không gửi email.`
          );
      ...
      console.log(
        `👍 Hoàn thành tác vụ gửi email loại "${type}" đến ${recipientEmail}.`
      );
