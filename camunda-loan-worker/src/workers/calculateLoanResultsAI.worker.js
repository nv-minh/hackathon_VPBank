  ...
  client.subscribe("calculateLoanResultsAI", async ({ task, taskService }) => {
    ...
    try {
      ...
      if (!customerData || !customerData.application_id) {
        throw new Error(
          "Không tìm thấy 'customerData' hoặc 'application_id' để gửi cho AI."
        );
      }
      ...
      console.log(
        "... Chuẩn bị gọi service AI với payload:",
      ...
      if (!aiResponse || typeof aiResponse.score === "undefined") {
        throw new Error(
          "Dữ liệu trả về từ AI không đúng định dạng hoặc không chứa 'score'."
        );
      }
      ...
      console.log(
        `✅ AI trả về score: ${score}. Chuyển đổi thành decision: '${decision}'.`
      );
      ...
      console.error(
        "❌ Lỗi trong worker [calculateLoanResultsAI]:",
        error.message
      );
