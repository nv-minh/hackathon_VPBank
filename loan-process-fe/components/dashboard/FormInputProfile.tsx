...
export function FormInputProfile({ onProfileCreated }: FormInputProfileProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.person_age || parseInt(formData.person_age) <= 0)
      newErrors.person_age = "Vui lòng nhập tuổi hợp lệ.";
    if (!formData.person_income || parseFloat(formData.person_income) <= 0)
      newErrors.person_income = "Vui lòng nhập thu nhập hợp lệ.";
    if (!formData.person_home_ownership)
      newErrors.person_home_ownership = "Vui lòng chọn tình trạng nhà ở.";
    if (
      !formData.person_emp_length ||
      parseFloat(formData.person_emp_length) < 0
    )
      newErrors.person_emp_length = "Vui lòng nhập số năm làm việc hợp lệ.";
    if (!formData.loan_intent)
      newErrors.loan_intent = "Vui lòng chọn mục đích vay.";
    if (!formData.loan_amnt || parseFloat(formData.loan_amnt) <= 0)
      newErrors.loan_amnt = "Vui lòng nhập số tiền vay hợp lệ.";
    if (!formData.loan_int_rate || parseFloat(formData.loan_int_rate) <= 0)
      newErrors.loan_int_rate = "Vui lòng nhập lãi suất mong muốn hợp lệ.";
    if (!formData.cb_person_default_on_file)
      newErrors.cb_person_default_on_file =
        "Vui lòng xác nhận lịch sử nợ quá hạn.";
    if (
      !formData.cb_person_cred_hist_length ||
      parseInt(formData.cb_person_cred_hist_length) < 0
    )
      newErrors.cb_person_cred_hist_length =
        "Vui lòng nhập lịch sử tín dụng hợp lệ.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      credit_history_length_years: parseInt(
        formData.cb_person_cred_hist_length
      ),
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error("Lỗi", {
          description: `Không thể tạo hồ sơ: ${
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="mb-2 text-xl font-bold text-red-600">
          Bạn chưa có hồ sơ vay vốn!
        </CardTitle>
        <CardDescription className="text-gray-700">
          Vui lòng điền thông tin dưới đây để tạo hồ sơ và bắt đầu đăng ký khoản
          vay.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          {" "}
          {/* Tăng khoảng cách để form thoáng hơn */}
          <div>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                />
                {errors.person_age && (
                  <p className="text-sm text-red-500">{errors.person_age}</p>
                )}
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={formatCurrency(formData.person_income)}
                  onChange={handleCurrencyChange}
                  className={errors.person_income ? "border-red-500" : ""}
                />
                {errors.person_income && (
                  <p className="text-sm text-red-500">{errors.person_income}</p>
                )}
              </div>
              <div className="space-y-2">
                ...
                <Select
                  onValueChange={(val) =>
                    handleSelectChange("person_home_ownership", val)
                  }
                  value={formData.person_home_ownership}
                >
                  <SelectTrigger
                    className={
                      errors.person_home_ownership ? "border-red-500" : ""
                    }
                  >
              ...
              <div className="space-y-2">
                <Label htmlFor="cb_person_default_on_file">
                  Từng có nợ quá hạn?
                </Label>
                <Select
                  onValueChange={(val) =>
                    handleSelectChange("cb_person_default_on_file", val)
                  }
                  value={formData.cb_person_default_on_file}
                >
                  <SelectTrigger
                    className={
                      errors.cb_person_default_on_file ? "border-red-500" : ""
                    }
                  >
              ...
              <div className="space-y-2">
                <Label htmlFor="cb_person_cred_hist_length">
                  Lịch sử tín dụng (số năm)
                </Label>
                <Input
                  ...
                  onChange={handleChange}
                  className={
                    errors.cb_person_cred_hist_length ? "border-red-500" : ""
                  }
                />
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={formatCurrency(formData.loan_amnt)}
                  onChange={handleCurrencyChange}
                  className={errors.loan_amnt ? "border-red-500" : ""}
                />
                {errors.loan_amnt && (
                  <p className="text-sm text-red-500">{errors.loan_amnt}</p>
                )}
              </div>
              <div className="space-y-2">
                ...
                <Select
                  onValueChange={(val) =>
                    handleSelectChange("loan_intent", val)
                  }
                  value={formData.loan_intent}
                >
                  <SelectTrigger
                    className={errors.loan_intent ? "border-red-500" : ""}
                  >
                ...
                {errors.loan_intent && (
                  <p className="text-sm text-red-500">{errors.loan_intent}</p>
                )}
              </div>
              <div className="space-y-2">
                <Label htmlFor="loan_int_rate">
                  Lãi suất mong muốn (%/năm)
                </Label>
                <Input
                  id="loan_int_rate"
                  type="number"
                  ...
                  onChange={handleChange}
                  className={errors.loan_int_rate ? "border-red-500" : ""}
                />
                {errors.loan_int_rate && (
                  <p className="text-sm text-red-500">{errors.loan_int_rate}</p>
                )}
              </div>
            </div>
          </div>
